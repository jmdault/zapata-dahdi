 ...
 * Note: if using FXO, set the RINGS variable to the number of rings
 * you want to wait before answering.
 * By default, it is set to zero to test FXS interfaces.
...
#define PLEASE_ENTER '5'

        /* The time (ms) to wait for a "5" from the caller */
#define TM_OUT  (9000)
...
void main()
{
  ...
  while (1) {
    ...
    if(RINGS>0) {
      ...
      /* Zero rings, to demo the FXS interface */
      printf("Waiting for off-hook... press CTRL-C to exit.\n");
      sleep(1);

      /* Initial state */
      result = ioctl(zap_fd(zp), DAHDI_GET_PARAMS, &params);
      ...
      if(!isoffhook) {
        /* Wait for event */
        for(;;) {
          result = zap_waitevent(zp);
          if(result==ZAP_EVENT_RINGANSWER) {
            break;
            printf("OK!\n");
          }
          sleep(0.2);
    ...
    result = zap_playf(zp, "sounds/vm-leavemsg.ulaw", ZAP_DTMFINT | ZAP_HOOKEXIT);
    err_if(result, '=', -1, __LINE__);   /* error if -1 */
